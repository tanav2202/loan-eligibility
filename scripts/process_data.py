...
class DataValidation:
    ...
    def __init__(self, df_train: pd.DataFrame, target_col: str):
        """
        DataValidation now works directly on df_train (NO file reading, NO splitting)
        """
    ...
    def _check_target_feature_correlations_v1(self, df, threshold=0.99):
    ...
    def _check_target_feature_correlations(self, df, threshold=0.99):
        """
        Check for extremely high correlations between numeric features and the (numeric) target column.
        Raises an error if any feature has abs(correlation) > threshold.
        """
        ...
        # Build temp dataframe for correlation calculation
        temp = df[numeric_features].copy()
        temp["_target_"] = y
...
def main(raw_data, write_to):
    """
    Read raw data, validate it, preprocess it, split it into train/test sets,
    scale the features, and save all outputs to disk.
    """
    ...
    # ---------------------------
    # 3. Save outputs to CSV
    # ---------------------------
    print("Saving processed datasets...")

    pd.DataFrame(X_train_scaled, columns=feature_names) \
        .to_csv(os.path.join(write_to, "X_train_scaled.csv"), index=False)

    pd.DataFrame(X_test_scaled, columns=feature_names) \
        .to_csv(os.path.join(write_to, "X_test_scaled.csv"), index=False)

    pd.DataFrame(y_train, columns=[target_col]) \
        .to_csv(os.path.join(write_to, "y_train.csv"), index=False)

    pd.DataFrame(y_test, columns=[target_col]) \
        .to_csv(os.path.join(write_to, "y_test.csv"), index=False)
    
    df_train.to_csv(os.path.join(write_to, "df_train.csv"), index=False)
    df_test.to_csv(os.path.join(write_to, "df_test.csv"), index=False)    

    print(f"Processed and split data saved in: {write_to}")
